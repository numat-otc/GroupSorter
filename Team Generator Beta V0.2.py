...
def MainPage():
    ...
    AddNameEntry = Text(root, width = 14, font = (DefFont, 27), bg=SecBack, fg=SecFore, bd=0, height = 1); AddNameEntry.place(relx=0.02, rely=0.806, anchor = W)
    AddNameEntry.focus()
    ...
    SortButton = Button(root, text="Sort", font = (DefFont, 32), bg=SecBack, fg='steel blue', bd=0, width=15, command = SortTeams, height = 2); SortButton.place(relx=0.18, rely=0.85, anchor = W)
...
def CreateName(i):
    ...
    globals()["PlayerNameButton" + str(i)] = Button(root, text=(NameList[i] + "\n"), font=(DefFont, 40), bg=DefBack, fg=DefFore, takefocus=False, activeforeground=SecFore, activebackground=DefBack, bd=0, command = lambda: RemovePlayer(i), width=10, anchor = W, height = 1)
...
def CreateTeam():
    ...
    for i in range(2):
        for i in range(3):
            TeamHorizontal.append(0.2 + (i * 0.22))
    for i in range(3):
        TeamVertical.append(0.2)
    for i in range(3):
        TeamVertical.append(0.55)
    globals()["TeamLabel" + str(TeamCount)] = Label(root, text=(""), font=(DefFont, 32), bg=DefBack, fg=DefFore, width = 16, height=6, anchor = N, bd=2, relief='groove'); globals()["TeamLabel" + str(TeamCount)].place(relx=TeamHorizontal[TeamCount], rely=TeamVertical[TeamCount], anchor = W)
    TeamCount += 1
